...
resource "aws_instance" "ec2ExpressApp" {
  ...
  tags = {
    Name = "f2i-autoInstance"
  }
  // provisionner la machine avec les commandes suivantes
  user_data = file("./init-script.sh")

  // définition de la connexion ssh
  connection {
    type        = "ssh"
    user        = "ec2-user"
    private_key = tls_private_key.ec2Key.private_key_pem
    host        = self.public_ip
  }
}
